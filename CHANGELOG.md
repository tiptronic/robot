## <small>0.7.1 (2024-06-27)</small>

* **FEAT**: Add `getScreens()` function for multi-monitor support
  - Returns array of all connected monitors with position and size information
  - Supports macOS, Linux (X11), and Windows platforms
  - Each screen object contains: `x`, `y`, `width`, `height`
* **FEAT**: Enhance `getScreenSize()` function with parameter support
  - `getScreenSize()` or `getScreenSize(0)`: Returns virtual screen bounds (all monitors combined)
  - `getScreenSize(1)`, `getScreenSize(2)`, etc.: Returns specific monitor information
  - Returns `null` for invalid parameters (negative numbers or non-existent monitors)
  - Virtual screen includes: `width`, `height`, `minX`, `minY`, `maxX`, `maxY`
  - Individual monitors include: `width`, `height`, `x`, `y`
* **FEAT**: Add comprehensive TypeScript definitions for new functionality
* **FEAT**: Add tests and examples for multi-monitor functionality
* **DOCS**: Update README with multi-monitor examples and documentation
