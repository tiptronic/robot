...
<p>Experimental Node.js Desktop Automation</p>
...
##### [Multiple Screens](https://github.com/octalmage/robotjs/wiki/Syntax#screen)
...
var screens = robot.getScreens();
console.log("Found " + screens.length + " screen(s):");

screens.forEach((screen, index) => {
    console.log(`Screen ${index + 1}: ${screen.width}x${screen.height} at (${screen.x}, ${screen.y})`);
});

// Move mouse to center of each screen.
screens.forEach((screen) => {
    var centerX = screen.x + Math.floor(screen.width / 2);
    var centerY = screen.y + Math.floor(screen.height / 2);
    robot.moveMouse(centerX, centerY);
});
```

##### [Enhanced Screen Size](https://github.com/octalmage/robotjs/wiki/Syntax#screen)

```JavaScript
// Get virtual screen bounds (all monitors combined).
var robot = require("robotjs");

// Get virtual screen bounds - no parameter or parameter 0
var virtualScreen = robot.getScreenSize(); // or robot.getScreenSize(0)
console.log(`Virtual screen: ${virtualScreen.width}x${virtualScreen.height}`);
console.log(`Bounds: (${virtualScreen.minX}, ${virtualScreen.minY}) to (${virtualScreen.maxX}, ${virtualScreen.maxY})`);

// Get specific monitor size - parameter 1, 2, 3, etc.
var monitor1 = robot.getScreenSize(1); // First monitor
var monitor2 = robot.getScreenSize(2); // Second monitor (if available)

console.log(`Monitor 1: ${monitor1.width}x${monitor1.height} at (${monitor1.x}, ${monitor1.y})`);

// Invalid parameters return null
var invalid = robot.getScreenSize(-1);  // null
var invalid2 = robot.getScreenSize(999); // null
```
...
The library now supports multi-monitor setups through the `getScreens()` function, which returns information about all connected displays including their position, width, and height. This allows you to work with multiple monitors in your automation scripts.
